...
if [ -z ${SERVICE_NETWORK+X} ]; then
    docker run -d -v /opt/jupyter:/home/jupyter:rw -e VIRTUAL_HOST=$DOMAIN -e LETSENCRYPT_HOST=$DOMAIN --network=$NETWORK --name $NAME --restart=always easy-cloud/jupyterhub
else
